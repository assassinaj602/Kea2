...
`u2_flutter` is a standalone plugin that extends Kea2 to enable testing of Flutter applications. It provides a high‑level `Flutter` driver that abstracts the Flutter widget tree, allowing you to write property‑based tests that interact directly with Flutter UI elements.
...
**Prerequisites**
- Flutter SDK (3.16+)
- A debug‑build of your Flutter app (APK or IPA)
- Python 3.7+ and `u2_flutter` installed (e.g., `pip install -e /path/to/u2_flutter`)
...
void main() async {
  // Enable the Flutter driver extension for testing.
...
**Integration with Kea2**
- Import `Flutter` and the `with_flutter` decorator in your Kea2 scripts.
- Use `self.flutter` within `@precondition`/`@prob` decorated methods just like the `Device` driver.

**Limitations**
- Only supports Android debug builds currently.
- UI element identification relies on widget `key` attributes; ensure they are set in your Flutter code.
